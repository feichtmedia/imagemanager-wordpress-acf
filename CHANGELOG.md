...
- Fixed: The plugin icons added in v1.2.0 were never shown in the WordPress.org plugin directory: they shipped inside the plugin's `assets/` directory (deployed to SVN `trunk/assets/`), but WordPress.org only reads directory assets from the SVN **top-level** `/assets/` directory. The icons were moved to a new `.wordpress-org/` directory that `release.yml` deploys via `ASSETS_DIR`, excluded from the plugin zip via `.distignore`.
- Fixed: The active-image highlight in the file browser modal was lost after navigating to another category and back, because `navigateToCategory()` / `navigateTo()` cleared the selection state. Navigation now preserves the selection — only an explicit image click changes it — and re-render matching compares canonical image IDs via a new `getImageId()` helper in `acf-imagemanager-field.js`.
- Fixed: GraphQL resolver returned `null` for `imagemanager_image` fields inside ACF Blocks, whose values live in the block's `attrs` in `post_content` instead of post meta. `resolve_field_value()` now mirrors WPGraphQL for ACF's own block handling — `acf_setup_meta()` on the block data, `get_field()` against the prefixed block ID, raw-attrs fallback — guarded by `function_exists('acf_prepare_block')` since ACF Blocks are PRO-only.
- Fixed: GraphQL resolver returned `null` for `imagemanager_image` fields nested inside an ACF Group: group values are passed down **unformatted** and keyed by field _key_, which the previous repeater-only resolver (field _name_, formatted values) did not recognise. The resolver logic in `includes/class-graphql.php` was extracted into a new `resolve_field_value()` method handling all `$root` shapes: name-keyed formatted values (repeaters, flexible content) used as-is, key-/`__key`-keyed raw values formatted via `acf_format_value()`, and top-level fields resolved via `\WPGraphQL\Acf\Utils::get_node_acf_id()` — which also fixes fields on terms, users, and options pages.
